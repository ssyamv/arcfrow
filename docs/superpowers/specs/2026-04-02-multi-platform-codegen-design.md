...
胶水服务需要新增对 Git MR merge 事件的监听，判断合并的分支是否匹配 `feature/{issue-id}-backend` 模式，匹配则触发第二轮。

### 5.3 对胶水服务数据模型的影响

`workflow_execution` 表已能支持两轮生成，通过 `workflow_type` 区分：
- 第一轮：`workflow_type = 'code_gen_backend'`
- 第二轮：`workflow_type = 'code_gen_frontend'`
